...
<form method="POST" action="{{ $formAction }}" enctype="multipart/form-data" class="space-y-4 rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
    @csrf
    @if($method === 'PUT')
        @method('PUT')
    @endif
    ...
    <div>
        <label class="mb-1 block text-xs font-semibold text-slate-600">مرفقات العقد</label>
        <input type="file" name="attachments[]" multiple class="w-full rounded-lg border-slate-300 text-sm">
        <p class="mt-1 text-[11px] text-slate-500">يمكن رفع حتى 10 ملفات (الحد الأقصى 10MB لكل ملف).</p>
    ...
    <div class="flex flex-wrap items-center gap-2">
        ...
        <a href="{{ route('workspace.contracts.index') }}" class="rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100">العودة</a>
    </div>
</form>
