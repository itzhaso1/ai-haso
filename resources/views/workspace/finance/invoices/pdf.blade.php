...
<body>
    <div class="card">
            ...
            <tr>
                <td style="width:55%">
                    ...
                    <p class="muted">
                        ...
                        ضريبة القيمة المضافة: {{ $setting?->vat_number ?? '-' }} | السجل التجاري: {{ $setting?->commercial_registration ?? '-' }}<br>
                ...
                <td style="text-align:left;">
                    ...
                    <p class="muted">تاريخ الإصدار: {{ $invoice->issue_date }}</p>
                    <p class="muted">تاريخ الاستحقاق: {{ $invoice->due_date ?? '-' }}</p>
                </td>
            </tr>
        </table>
        ...
        <table style="margin-top:16px;">
            <tr>
                <td>
                    <strong>العميل:</strong> {{ $invoice->customer?->name ?? $invoice->customer_name ?? '-' }}<br>
        ...
        <table class="totals" style="margin-top:12px;">
            <tr><td style="width:70%"></td><td>الإجمالي قبل الضريبة:</td><td>{{ number_format((float) $invoice->subtotal, 2) }}</td></tr>
            <tr><td></td><td>الخصم:</td><td>{{ number_format((float) $invoice->discount, 2) }}</td></tr>
            <tr><td></td><td>المبلغ الخاضع للضريبة:</td><td>{{ number_format((float) $invoice->taxable_amount, 2) }}</td></tr>
            <tr><td></td><td>ضريبة القيمة المضافة:</td><td>{{ number_format((float) $invoice->tax_amount, 2) }}</td></tr>
            <tr><td></td><td><strong>الإجمالي:</strong></td><td><strong>{{ number_format((float) $invoice->total, 2) }}</strong></td></tr>
            <tr><td></td><td>المدفوع:</td><td>{{ number_format((float) $invoice->amount_paid, 2) }}</td></tr>
            <tr><td></td><td>المتبقي:</td><td>{{ number_format((float) $invoice->amount_due, 2) }}</td></tr>
        </table>
        ...
        <div style="margin-top:14px;">
            <strong>بنية ZATCA و QR:</strong>
            <p class="muted">
                UUID: {{ $invoice->zatca_uuid ?? 'N/A' }}<br>
                XML Hash: {{ $invoice->zatca_xml_hash ?? 'N/A' }}<br>
                QR Payload: {{ $invoice->zatca_qr_code ? 'متوفر' : 'غير منشأ بعد' }}
            </p>
        </div>
    </div>
</body>
