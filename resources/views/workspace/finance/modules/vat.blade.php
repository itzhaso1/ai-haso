    ...
    <div class="space-y-4">
        ...
        <div class="grid gap-3 sm:grid-cols-3">
            <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <p class="text-xs text-slate-500">Output VAT</p>
                <p class="mt-2 text-2xl font-bold">{{ number_format((float) $vat['output'], 2) }}</p>
            </article>
            <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <p class="text-xs text-slate-500">Input VAT</p>
                <p class="mt-2 text-2xl font-bold">{{ number_format((float) $vat['input'], 2) }}</p>
            </article>
            <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <p class="text-xs text-slate-500">Net VAT</p>
                <p class="mt-2 text-2xl font-bold">{{ number_format((float) $vat['net'], 2) }}</p>
            </article>
        </div>

        <article class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
            <h3 class="mb-3 text-sm font-bold">Tax Engine Profiles</h3>
            <div class="grid gap-2 md:grid-cols-2">
                @foreach($rates as $rate)
                    <div class="rounded-lg border border-slate-200 p-3 text-sm">
                        <p class="font-semibold">{{ $rate->name }} @if($rate->is_default)<span class="text-xs text-[#06C2A4]">(Default)</span>@endif</p>
