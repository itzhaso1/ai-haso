@php
    ...
    $max = max(1, (float) $points->max($valueKey));
@endphp
<div class="space-y-2">
    @forelse($points as $point)
        ...
        <div>
            <div class="mb-1 flex items-center justify-between gap-3 text-xs text-slate-500">
                ...
                <span class="shrink-0 font-semibold text-slate-900">{{ number_format($amount, 2) }}</span>
            </div>
            <div class="h-2.5 overflow-hidden rounded-full bg-slate-100">
                <div class="h-2.5 rounded-full bg-[#06C2A4]" style="width: {{ $width }}%"></div>
            </div>
        </div>
    @empty
