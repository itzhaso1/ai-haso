<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between gap-3">
            <h2 class="text-xl font-semibold text-gray-900">Email CRM & Inbox Hub</h2>
            <span class="rounded-full bg-[#E8FAF6] px-3 py-1 text-xs font-semibold text-[#0f7668]">
                Workspace Isolated
            </span>
        </div>
    </x-slot>
            ...
            <div class="grid gap-4 xl:grid-cols-[320px_420px_1fr]">
                <aside class="space-y-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    ...
                    <details class="group rounded-xl border border-slate-200">
                        ...
                        <form method="POST" action="{{ route('workspace.emails.accounts.store') }}" enctype="multipart/form-data" class="space-y-2 p-3">
                            @csrf
                            <input name="name" placeholder="الاسم التعريفي" class="w-full rounded-lg border-slate-300 text-sm" required>
                            <input type="email" name="email" placeholder="company@example.com" class="w-full rounded-lg border-slate-300 text-sm" required>
                            <input type="password" name="password" placeholder="كلمة المرور" class="w-full rounded-lg border-slate-300 text-sm" required>
                    ...
                    @if($currentAccount)
                        <details class="group rounded-xl border border-slate-200" open>
                            ...
                            <form method="POST" action="{{ route('workspace.emails.accounts.update', $currentAccount) }}" enctype="multipart/form-data" class="space-y-2 p-3">
                                @csrf
                                @method('PUT')
                                <input name="name" value="{{ $currentAccount->name }}" class="w-full rounded-lg border-slate-300 text-sm" required>
                ...
                <section class="space-y-4">
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        <h3 class="mb-3 text-sm font-semibold text-slate-900">Thread View</h3>
                    ...
                    <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                        ...
                        @if($currentAccount)
                            <form method="POST" action="{{ route('workspace.emails.messages.send') }}" enctype="multipart/form-data" class="space-y-3">
                                ...
                                <div>
                                    <label class="mb-1 block text-xs font-semibold text-slate-600">المرفقات</label>
