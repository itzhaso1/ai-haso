                ...
                child: NotificationListener<ScrollNotification>(
                  ...
                  child: ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      final time = c.lastMessageAt == null ? '' : DateFormat('HH:mm').format(c.lastMessageAt!.toLocal());
                      return ListTile(
                        onTap: () => context.push('/conversations/${c.id}'),
                        leading: CircleAvatar(
                          backgroundColor: Theme.of(context).colorScheme.primary.withValues(alpha: 0.15),
                          child: Text((c.title.isNotEmpty ? c.title[0] : '#'), style: TextStyle(color: Theme.of(context).colorScheme.primary, fontWeight: FontWeight.bold)),
                        ),
                        title: Row(
                          children: [
                            Expanded(child: Text(c.title, maxLines: 1, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: c.unreadCount > 0 ? FontWeight.w800 : FontWeight.w600))),
                        ...
                        subtitle: Row(
                          children: [
                            ...
                            if (c.unreadCount > 0)
                              CircleAvatar(radius: 11, backgroundColor: Theme.of(context).colorScheme.primary, child: Text('${c.unreadCount}', style: const TextStyle(color: Colors.white, fontSize: 11))),
                          ],
                        ),
                      );
                    },
